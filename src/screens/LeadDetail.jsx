...
export default function LeadDetail() {
  ...
  const [msgs, setMsgs] = useState([
    { id: 1, from: 'evo', text: `EVO התאים את האירוע הזה לפרופיל שלך. ציון התאמה: ${lead?.matchScore}%. הנה הבריף המלא.`, time: lead?.receivedAt },
  ])
  ...
  const sendMsg = () => {
    ...
    setMsg('')
    setTimeout(() => {
      setMsgs(prev => [...prev, { id: Date.now() + 1, from: 'evo', text: "קיבלתי — אעביר זאת ללקוח ואעדכן אותך בקרוב.", time: 'עכשיו' }])
    }, 1200)
  }
      ...
      <div className="px-6 space-y-5 pt-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            ['תאריך', lead.date],
            ['אורחים', `${lead.guestCount} אנשים`],
            ['מיקום', lead.location],
            ['תקציב', lead.budgetRange],
            ['פג תוקף', lead.expiresIn ? `בעוד ${lead.expiresIn}` : '—'],
          ].map(([k, v]) => (
        ...
        {/* Messages */}
        <div>
          <p className="text-xs font-bold tracking-[0.2em] uppercase text-evo-muted mb-4">ערוץ EVO</p>
          ...
          <div className="flex gap-3">
            <input value={msg} onChange={e => setMsg(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMsg()}
              placeholder="שאל את EVO על הליד הזה..."
